...
const admin: TranslationStrings = {
  ...
  'admin.apiKeysHint': 'Optional. An Amap key is used first for China place search, tiles, routes and weather. A Google key adds photos, ratings and opening hours.',
  'admin.amapKey': 'Amap (Gaode) Web API Key',
  'admin.amapKeyHintLong': 'When set, place search, autocomplete, details and reverse geocoding try Amap first. Empty or failed results fall through to OpenStreetMap (Google is skipped). Get a Web service key at console.amap.com. Do not put this key in the Google field.',
  'admin.mapsKey': 'Google Maps API Key',
  'admin.mapsKeyHint': 'Required for place search. Get at console.cloud.google.com',
  'admin.mapsKeyHintLong': 'Used when no Amap key is set. Without a Google or Amap key, OpenStreetMap is used for place search. A Google key can also load photos, ratings and opening hours. Get one at console.cloud.google.com.',
