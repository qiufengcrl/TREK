...
const admin: TranslationStrings = {
  ...
  'admin.apiKeysHint': '可选。配置高德 Key 后，国内地点搜索、底图、路线和天气会优先走高德。Google Key 用于照片、评分和营业时间。',
  'admin.amapKey': '高德 Web 服务 Key',
  'admin.amapKeyHintLong': '配置后，地点搜索、补全、详情和逆地理会优先走高德。空结果或失败会回落到 OpenStreetMap（不再尝试 Google）。请在 console.amap.com 申请 Web 服务 Key，不要填进 Google 栏。',
  'admin.mapsKey': 'Google Maps API 密钥',
  'admin.mapsKeyHint': '用于地点搜索。在 console.cloud.google.com 获取',
  'admin.mapsKeyHintLong': '仅在未配置高德 Key 时使用。没有 Google 或高德 Key 时用 OpenStreetMap 搜索。Google Key 还可加载照片、评分和营业时间。在 console.cloud.google.com 获取。',
