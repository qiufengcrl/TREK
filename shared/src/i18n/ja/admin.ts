...
const admin: TranslationStrings = {
  ...
  'admin.apiKeys': 'APIキー',
  'admin.apiKeysHint': '任意。Amap キーがあると中国での場所検索、タイル、ルート、天気を優先します。Google キーは写真・評価・営業時間を追加します。',
  'admin.amapKey': 'Amap（高徳）Web APIキー',
  'admin.amapKeyHintLong': '設定すると、場所検索・補完・詳細・逆ジオコーディングは先に Amap を使います。空または失敗時は OpenStreetMap にフォールバックします（Google は使いません）。Webサービスキーは console.amap.com。Google 欄には入れないでください。',
  'admin.mapsKey': 'Google Maps APIキー',
  'admin.mapsKeyHint': '場所検索に必要。console.cloud.google.com で取得',
  'admin.mapsKeyHintLong': 'Amap キーがないときだけ使います。Google / Amap キーがなければ OpenStreetMap で検索します。Google キーがあれば写真、評価、営業時間も表示できます。console.cloud.google.com で取得。',
