...
const places: TranslationStrings = {
  ...
  'places.osmHint': 'OpenStreetMapで検索しています（写真・営業時間・評価なし）。中国向けは Amap キー、評価と営業時間には Google キーを追加してください。',
  'places.osmActive': 'OpenStreetMapを使用中。中国では Amap キーが適しています。Google APIキーで評価と営業時間が追加されます。',
  'places.amapActive': '場所検索に Amap（高徳）を使用中',
  ...
  'places.details.noKeyHint': 'Google API キーがない場合、評価・営業時間・写真は自由に使えるソースのみです。TREK の管理者に Google キー、または中国検索用の Amap キーの追加を依頼してください。',
