...
describe('amap.client', () => {
  ...
  it('AMAP-004: details reads a POI id and a coord placeholder', async () => {
    ...
    expect(named?.lat).toBeCloseTo(39.907, 5);
    expect(named?.lng).toBeCloseTo(116.391, 5);
  ...
  it('AMAP-005: share-URL helpers accept Amap hosts and reject look-alikes', () => {
    ...
    expect(pos?.lat).toBeLessThan(39.908823);
  });
  ...
  it('AMAP-007: parses the App share landing ?p=poiid,lat,lng,name,address', () => {
    ...
    expect(pos?.lat).toBeCloseTo(31.14, 2);
    expect(pos?.lng).toBeCloseTo(121.11, 2);
    expect(pos?.lng).toBeLessThan(121.11573144793509);
    expect(extractAmapName(url)).toBe('青溪园');
  });

  it('AMAP-006: search and autocomplete write a GCJ location= from WGS bias', async () => {
