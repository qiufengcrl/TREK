...
/**
 * Search row of the place form: Amap/Google/OSM text search biased on the trip
 * centre, autocomplete dropdown, plus share-URL and "lat, lng" paste
...
export default function PlPlaceSearch({ planner, locationBias, onPick, onResolvingChange }: PlPlaceSearchProps) {
  ...
  const { hasMapsKey, hasAmapKey } = useAuthStore()
  ...
  return (
    <div className="relative">
      {hasAmapKey ? (
        <p className="mb-2 text-[0.6875rem] text-m-muted">{t('places.amapActive')}</p>
      ) : !hasMapsKey ? (
        <p className="mb-2 text-[0.6875rem] text-m-muted">{t('places.osmActive')}</p>
      ) : null}
