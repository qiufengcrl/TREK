...
import { isTiandituTileUrl, isVectorStyle, normalizeTileUrl, resolveTileUrl, tiandituLabelUrl, withTileApiKey, withTiandituKey } from '../utils/tileUrl'
import { OFM_POSITRON, TIANDITU_SUBDOMAINS } from '../constants/mapDefaults'
...
export async function prefetchTiles(
  ...
  async function worker(): Promise<void> {
    while (cursor < coords.length) {
      ...
      const labelTemplate = tiandituLabelUrl(tileUrlTemplate)
      const labelUrl = labelTemplate ? buildTileUrl(labelTemplate, z, x, y, cartoKey, tiandituKey) : null

      if (!(cache && (await cache.match(url)))) {
        await fetch(url, { mode: 'no-cors' }).catch(() => {})
        fetched++
      }
      if (labelUrl && !(cache && (await cache.match(labelUrl)))) {
...
export async function prefetchTilesForTrip(
  ...
  const template = resolveTileUrl(tileUrlTemplate, DEFAULT_TILE_URL, cartoKey, tiandituKey)
