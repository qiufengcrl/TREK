...
| `AMAP_API_KEY`            | Optional [Amap (高德) Web service](https://console.amap.com/) key. When set, place search / autocomplete / details / reverse geocoding try Amap first, then Google, then Nominatim (coordinates converted GCJ-02 → WGS-84). Do not put this key in the Google field. Can also be set in Admin → Settings → API Keys. | — |
| `TIANDITU_API_KEY`        | Optional [Tianditu](https://console.tianditu.gov.cn/) browser `tk` for WGS-84 raster tiles. Also settable in Settings → Map. | — |
