...
| `AMAP_API_KEY`            | Optional [Amap (高德) Web service](https://console.amap.com/) key. When set, place search / autocomplete / details / reverse geocoding try Amap first, then Nominatim (Google is skipped). Coordinates are stored as **WGS-84**; TREK converts at the Amap boundary. Do not put this key in the Google field. Can also be set in Admin → Settings → API Keys. | — |
